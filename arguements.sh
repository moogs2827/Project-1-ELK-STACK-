...
#
#Check for a n output file
if [ ! $1 ]
then
  echo "Please specifiy an output file."
  exit
fi
...
usermod -L root
#Print it out and log it
echo $(date) Disabled root schell. Root user cannot login." | tee -a $1
...
END
#Print it out and log it
echo "$(date) Added custom aliases collection to sysadmin's bashrc." | tee -a $1
#Print it out and log exit
